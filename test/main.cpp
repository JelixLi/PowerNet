...
#ifdef GPU
    auto Kernel = Power::Get_GemmKernel();
    Power::Init_Kernel(&Kernel);
#endif
...
int run() {
    ...
    int thread_num = 3;
    if (Power::Gemmer::gemmers.size() == 0) {
        for (int i = 0; i < max(thread_num, 3); i++) {
            Power::Gemmer::gemmers.push_back(new Power::Gemmer());
        }
    }
    Power::Loader *loader = Power::Loader::shared_instance();
    ...
    // std::string prefix("/home/jelix/code/PowerNet/PowerNet/test/model/squeezenet/");
    std::string prefix("/home/pi/PowerNet/test/model/squeezenet/");
    auto t1 = Power::time();
    ...
    Power::Net *net = new Power::Net(loader->_model);
    net->set_thread_num(thread_num);


    auto t2 = Power::time();
    cout << "load time : " << Power::time_diff(t1, t2) << "ms" << endl;
    ...
    for (int i = 0; i < count; i++) {
        ...
        Time t2 = Power::time();
        double diff = Power::time_diff(t1, t2);
        total += diff;
    }
